    ...
    while offset < len(data):
        ...
        if obj_id not in BTHOME_OBJECT_IDS:
            ...
            result["measurements"].append(
                {"type": f"unknown_0x{obj_id:02X}", "raw": data[offset:].hex()}
            )
            break
...
def parse_ad_structures(data: bytes) -> list:
    ...
    while offset < len(data):
        ...
        if ad_type == 0x01:  # Flags
            ...
            flag_list = []
            if flags & 0x01:
                flag_list.append("LE Limited Discoverable")
            if flags & 0x02:
                flag_list.append("LE General Discoverable")
            if flags & 0x04:
                flag_list.append("BR/EDR Not Supported")
            if flags & 0x08:
                flag_list.append("LE+BR/EDR Controller")
            if flags & 0x10:
                flag_list.append("LE+BR/EDR Host")
...
def decode_ble_packet(pkt, pkt_num: int):
    ...
    result: dict = {"packet_num": pkt_num}
    ...
    if hasattr(pkt, "load"):
        result["payload"] = (
            pkt.load.hex() if isinstance(pkt.load, bytes) else str(pkt.load)
        )
